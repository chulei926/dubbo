...
/**
 * AbstractBeanDefinitionParser：dubbo 配置文件解析器。
 *
 * @export
 */
public class DubboBeanDefinitionParser implements BeanDefinitionParser {
	...
	public DubboBeanDefinitionParser(Class<?> beanClass, boolean required) {
		System.out.println(">>>>> DubboBeanDefinitionParser >>> 创建 dubbo 配置文件解析器 >>>  " + beanClass.getName());
	 ...
	 * 解析 dubbo 配置文件的各种标签。
	 * <p>
	 * 将解析到的 dubbo 标签，封装成对应的 Bean（在 DubboNamespaceHandler 中初始化时注册），保存到 spring 容器。
	 * </p>
	 *
	 */
	private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
		...
		if (StringUtils.isNotEmpty(id)) {
			...
			// 将解析到的标签封装成对应的 bean，注册到 spring 容器中。
